...
delete LUONG where MaLuong = '';


SELECT 
    sp.TenSP AS [TenSP], 
    sl.SoLuong AS [SoLuong]
FROM 
    SanPham sp
LEFT JOIN 
    NHAKHO sl
ON 
    sp.MaSP = sl.MaSP;
select * from SANPHAM where MaSP = 'SP1'
 set dateformat dmy
 go
create proc ThemNV( @Manv nvarchar(10), @Tennv nvarchar(30), @NgaySinh date, @Diachi nvarchar(30),@SDTNhanVien char(15), @Phai char(5))
as
  insert into NHANVIEN(Manv,Tennv,NgaySinh,Diachi,SDTNhanVien,Phai)
  values(@Manv,@Tennv,@NgaySinh,@Diachi,@SDTNhanVien,@Phai)
go
     --exec ThemNV N'NV06',N' Phạm Hoài Hùng','13/11/2004',N'Thủ Đức','0987096433',N'Nam'
go
select* from NHANVIEN
go
...
AS
 ...
 go
Create Proc ThemKHACHHANG @MaKH nvarchar(10),@TenKH nvarchar(30),@DiaChiKH nvarchar(30), @SDTKH char(15) 
as
...
go
 ...
 --Thêm dữ liệu NCC
 go
 Create Proc ThemNCC @Manhacungcap nvarchar(10),@Tennhacungcap nvarchar(30),@Diachi nvarchar(30),@Sodienthoai char(15)
 as
  ...
  --Cập nhật NCC
  go
  Create Proc CapNhapNCC @sManhacungcap nvarchar(10),@Tennhacungcap nvarchar(30),@Diachi nvarchar(30),@Sodienthoai char(15)
  as
...
GO
 ...
 go
  ...
  --Cập nhật SP
  go
  Create Proc CapNhapSP(@sMaSP nvarchar(10),@sTenSP nvarchar(30), @sGiaSP float,@Manhacungcap nvarchar(10))
  as
...
go
   ...
   --Store thêm sản phẩm
go
CREATE PROCEDURE ThemSanPham @MaSP nvarchar(10), @TenSP nvarchar(30), @GiaSP float,@Manhacungcap nvarchar(10)
AS
...
--sua san pham 
go
    Create Proc SuaSanPham @MaSP nvarchar(10), @TenSP nvarchar(30), @GiaSP float,@Manhacungcap nvarchar(10)
  as
...
create proc ThemCHCIRCLEK @MaCH nvarchar(10), @DChi nvarchar(30), @GiayPhep nvarchar(30)
as
...
AS  
BEGIN  
    SET NOCOUNT ON;  -- Chặn thông tin trả về không cần thiết  
    ...
    FROM   
        NHANVIEN  
    INNER JOIN   
        LUONG ON NHANVIEN.Manv = LUONG.Manv  
    WHERE   
        NHANVIEN.Manv = @Manv;  
    ...
    IF @@ROWCOUNT = 0  
    BEGIN  
        PRINT 'Không tìm thấy nhân viên với mã: ' + @Manv;  -- Thông báo không tìm thấy  
    END  
END;
GO
...
AS  
BEGIN  
    SET NOCOUNT ON;  -- Ngăn chặn thông báo số lượng bản ghi bị ảnh hưởng  

    -- Kiểm tra nếu @MaHD là NULL hoặc rỗng  
    IF @MaHD IS NULL OR LTRIM(RTRIM(@MaHD)) = ''  
    BEGIN  
        PRINT 'Mã hóa đơn không hợp lệ.';  
        RETURN;  -- Kết thúc thủ tục nếu mã không hợp lệ  
    END  
    ...
    IF @@ROWCOUNT = 0  
    BEGIN  
        PRINT 'Không tìm thấy hóa đơn với mã: ' + @MaHD;  -- Thông báo không tìm thấy  
    END  
END;
go


--drop proc LayDSHoaDonTheoMaHD
create proc LayDSHoaDonTheoMaHD(@maHD nvarchar (10))
as 
select 
	hd.MaHD,   
	hd.MaKH,
	...
	from HOADON hd, ChiTietHoaDon cthd ,SANPHAM sp,KHACHHANG kh
	where hd.MaHD = @maHD and hd.MaHD = cthd.MaHD and cthd.MaSP = sp.MaSP and hd.MaKH = kh.MaKH
	
	--exec LayDSHoaDonTheoMaHD 'DH000014'
	--exec LayDSHoaDonTheoMaHD 'DH000015'
...
IF @@ROWCOUNT = 0  
BEGIN  
    PRINT 'Không tìm thấy hóa đơn với mã: DH000012';  -- Thông báo không tìm thấy nếu không có hóa đơn  
END
go
--report thong ke san pham
...
ncc.Tennhacungcap
from 
SANPHAM sp,NHACUNGCAP ncc, NHAKHO nk
where 
sp.MaSP = nk.MaSP and sp.Manhacungcap = ncc.Manhacungcap
